...
const RequestTabs = () => {
  ...
  const pendingRequests = requests.filter((request) => request.endorsement_ticket_id === null && request.status !== "Withdrawn");
  const ongoingRequests = requests.filter((request) => request.endorsement_ticket_id !== null && request.status === "Pending");
  const finishedRequests = requests.filter((request) => request.status !== "Pending");
    ...
    <Tabs className="w-full" defaultValue="pending">
      <TabsList className="flex w-full justify-center border-b h-15">
        <TabsTrigger className="py-2 px-4 font-medium" value="pending">
          Pending Requests
          <Badge className="ml-2 rounded-full bg-red-700 px-2 text-xs font-medium ">{pendingRequests.length}</Badge>
        </TabsTrigger>
        <TabsTrigger className="py-2 px-4 font-medium" value="ongoing">
          Ongoing Requests
          <Badge className="ml-2 rounded-full bg-red-700 px-2 text-xs font-medium ">{ongoingRequests.length}</Badge>
        </TabsTrigger>
        <TabsTrigger className="py-2 px-4 font-medium" value="finished">
          Finished Requests
          <Badge className="ml-2 rounded-full bg-red-700 px-2 text-xs font-medium ">{finishedRequests.length}</Badge>
        </TabsTrigger>
      </TabsList>
      <TabsContent value="pending">
        <div className="container mx-auto p-4">
        <h1 className="text-2xl font-bold mb-4">Pending Requests</h1>
        <DataTable data={pendingRequests} />
    </div>
      </TabsContent>
